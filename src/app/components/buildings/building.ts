...
export class Building extends Phaser.Physics.Arcade.Sprite {
    constructor (scene: Phaser.Scene, x: number, y: number, baseTileIdx: number, size: Vector2, foreground: BoundingBox, colliderPosition: BoundingBox, externalCallback: Function) {
        ...
        let originTile = (this.scene as WorldScene).layerObjectsBackground.getTileAt(mapPosition.x, mapPosition.y, true);
        let worldPosition = new Phaser.Math.Vector2(originTile.getLeft(this.scene.cameras.main), originTile.getTop(this.scene.cameras.main));
    ...
    updateTiles(mapPosition: Vector2, size: Vector2, foreground: BoundingBox, baseTileIdx: number){
        for(let i=0;i<size.x;i+=1){
            for(let j=0;j<size.y;j+=1){
                let layer = i >= foreground.x && i <= foreground.x + foreground.width && j >= foreground.y && j <= foreground.y + foreground.height ? (this.scene as WorldScene).layerObjectsForeground : (this.scene as WorldScene).layerObjectsBackground;
                layer.putTileAt(baseTileIdx + i + j * 32, mapPosition.x + i, mapPosition.y + j);
            }
        }
    }
}